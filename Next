...
def create_page_for_cookie(coki, page_name, user_agent):
    cookie_str = coki.strip()
    ...
    try:
        ...
        result['error'] = f"POST Error: {str(e)[:50]}"
        return result
    ...
    try:
        ...
        if additional_profile_id or page_id:
            ...
            result['error'] = str(reason)[:250] if reason else 'Unknown error'
    except Exception as e:
...
def create_pages():
    ...
    with ThreadPoolExecutor(max_workers=THREADS) as executor:
        ...
        for cookie, name in zip(cookies, page_names):
            futures.append(executor.submit(create_page_for_cookie, cookie, name.strip(), user_agent))
